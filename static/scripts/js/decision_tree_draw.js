function draw_decision_tree(data) {
    ...
    function graphicviz() {
        require.config({
            paths: {
                d3: "https://cdnjs.cloudflare.com/ajax/libs/d3/3.4.0/d3.min"
            }
        });
        ...
        require(["d3"], function (d3) {
            ...
            //Call The Function
            console.log("Decision tree file")
            var label_names;
            ...
            d3.json(filetochoose, function (error, flare) {
                ...
                tree = d3.layout.tree()
                    ...
                    .size([height, getDepth(flare) * width / 15]);

                diagonal = d3.svg.diagonal()
                    .projection(function (d) { return [d.y, d.x]; });

                svg = d3.select("#chartBox").append("svg")
                    .attr("width", getDepth(flare) * width / 15 + margin.right + margin.left)
            ...
            function update(source, n_labels) {
                ...
                nodeEnter.append("svg:image")
                    .attr("xlink:href", function (d) { return ((d.children || d._children) || version2) && d.type == 'categorical' ? catPng : numPng })
                    .attr("x", "-76")
                    .attr("y", "-74")
                    .attr("width", "20")
                    .attr("height", "20")
                ...
                nodeEnter.append("text")
                    .attr("x", function (d) { d.name = d.name ? d.name : ""; return (d.children || d._children) || version2 ? -75 - ((getTextWidth(d.name.replace(d.name.match(/(.*?)(<|>|=|!=|<=|>=|=<|=>)/i)[1], '').replace('=', ''), 14, 'Verdana') + 5.7) - (133 + 8)) / 2 : 0 })
                    .attr("y", -45)
                    .attr("dy", ".35em")
                    .attr("text-anchor", "start")
                    .style("font-size", "12px")
